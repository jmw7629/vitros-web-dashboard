    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm mt-0.5" style={{ color: theme.textSecondary }}>
          Select an analyzer to update its stage
        </p>
      </div>
      ...
      {!selected ? (
        ...
        <>
          ...
          <WebCard className="p-4">
            ...
            >
              {selected.analyzerType} · Current: {selected.currentStage}
            </div>
            <ProgressBar
              value={selected.overallPct}
              maxValue={100}
              color="#6366f1"
              height={8}
            />
            ...
            <div className="space-y-1.5">
              {STAGES.map(stage => (
                <button
                  ...
                  style={{
                    backgroundColor:
                      newStage === stage ? "#6366f122" : "transparent",
                ...
                >
                  <span
                    ...
                    style={{
                      backgroundColor:
                        newStage === stage ? "#6366f1" : theme.cardBorder,
                    }}
                  />
                  <span
                    className="text-sm"
          ...
          <WebCard className="p-4">
            ...
            <textarea
              id="rem-notes"
              className="w-full mt-1 px-3 py-2 rounded-xl text-sm border resize-none h-20"
              style={{
          ...
          {submitError && (
            <div className="text-xs text-red-400 px-1">{submitError}</div>
          )}

          <button
            disabled={newStage === selected.currentStage || isSubmitting}
            onClick={async () => {
              if (!selected) return;
              setIsSubmitting(true);
              setSubmitError(null);
              try {
                await data.updateAnalyzer(selected._id, {
