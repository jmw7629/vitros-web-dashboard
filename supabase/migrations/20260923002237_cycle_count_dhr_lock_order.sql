-- Forward replacement: only the DHR table lock mode changes; all inventory,
-- optimistic revision, idempotency, audit and service-role boundaries remain.
create or replace function public.apply_cycle_count_operation(p_operation text,p_payload jsonb,p_actor text,p_correlation_id uuid)
 ...
 if p_operation='createSchedule' then
  ...
  if p_operation in ('updateSchedule','deleteSchedule') then
   ...
   if p_operation='confirm' then
    ...
    -- Freeze DHR writers before taking any DHR row or stock locks. SHARE is
    -- insufficient: a lifecycle writer can hold a row (ROW SHARE table lock),
    -- then wait to UPDATE while this count waits for that same row. EXCLUSIVE
    -- waits for those row lockers too, avoiding the lock-upgrade deadlock.
    -- Ordinary SELECT readers remain concurrent; writers resume at commit.
