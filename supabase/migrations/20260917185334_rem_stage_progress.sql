-- Service-only REM progress, preserving canonical operational tables and imported quantities.
alter table public.rem_analyzers add column progress_revision bigint not null default 0,
 add column progress_updated_at timestamptz, add column progress_engineer_name text;
alter table public.rem_lvcc add column progress_revision bigint not null default 0,
 add column progress_updated_at timestamptz, add column progress_engineer_name text,
 add column operator_notes text check (length(operator_notes)<=4000);
create function public.bump_rem_progress_revision() returns trigger language plpgsql
...
create trigger rem_analyzer_progress_revision before update on public.rem_analyzers
for each row execute function public.bump_rem_progress_revision();
create trigger rem_lvcc_progress_revision before update on public.rem_lvcc
for each row execute function public.bump_rem_progress_revision();
revoke all on function public.bump_rem_progress_revision() from public,anon,authenticated;

create table public.rem_progress_events (
 id uuid primary key default gen_random_uuid(), kind text not null check(kind in ('analyzer','lvcc')),
 record_id uuid not null, revision bigint not null, engineer_id uuid not null,
 engineer_name text not null, actor text not null, created_at timestamptz not null default clock_timestamp(),
 correlation_id text not null unique, request jsonb not null, before_value jsonb, after_value jsonb not null
);
create index rem_progress_events_record on public.rem_progress_events(kind,record_id,revision desc);
alter table public.rem_progress_events enable row level security;
revoke all on public.rem_progress_events from public,anon,authenticated;
...
jsonb_build_object('procurementPct',p_row->'procurement_pct','cleaningPct',p_row->'cleaning_pct','servicePct',p_row->'service_pct','finalLinePct',p_row->'final_line_pct','packagingPct',p_row->'packaging_pct','releaseTestingPct',p_row->'release_testing_pct','qaReleasePct',p_row->'qa_release_pct','sapReleasePct',p_row->'sap_release_pct') else
...
create function public.apply_rem_progress_update(p_kind text,p_record_id uuid,p_expected_revision bigint,
...
begin
 ...
 select * into emp from public.convex_employees where id=p_engineer_id and active=true for share;
...
create function public.create_rem_lvcc_record(p_serial text,p_type text,p_batch text,p_engineer_id uuid,p_notes text,p_actor text,p_correlation_id text)
returns jsonb language plpgsql security invoker set search_path=pg_catalog,public as $$
