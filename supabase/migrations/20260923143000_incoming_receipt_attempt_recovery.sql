...
create table if not exists public.incoming_receipt_attempts (
  ...
  state text not null check (state in ('reviewed','attempted','accepted','unknown','conflict','abandoned')),
...
begin
  ...
  if not found or v_before.actor is distinct from v_actor then raise exception 'Receipt attempt is not available'; end if;
  if p_expected_revision is distinct from v_before.revision then raise exception 'Receipt attempt revision is stale'; end if;
  if v_before.state not in ('reviewed','conflict') then
    raise exception 'Receipt attempt cannot be abandoned after execution may have started';
  end if;
  if v_before.receipt is not null
     or exists (select 1 from public.inventory_operations where correlation_id=v_before.correlation_id)
    ...
    raise exception 'Receipt attempt has accepted or possibly accepted inventory movement and cannot be abandoned';
  end if;
  update public.incoming_receipt_attempts
     set state='abandoned', revision=revision+1, updated_at=now()
   where id=v_before.id returning * into v_attempt;
  insert into public.incoming_receipt_attempt_events
    (attempt_id,revision,event_type,from_state,to_state,actor,details)
  values (v_attempt.id,v_attempt.revision,'abandoned',v_before.state,'abandoned',v_actor,
    jsonb_build_object('correlationId',v_attempt.correlation_id,'reason','operator_resolution'));
  return public.incoming_receipt_attempt_payload(v_attempt);
end;
$$;
...
create or replace function public.reserve_incoming_manual_receipt_review(
  p_actor text,
  p_document_ref text,
  p_part_number text,
  p_qty integer
...
begin
  ...
  -- One lock per physical document makes concurrent clients serialize before a line identity is returned.
  perform pg_advisory_xact_lock(hashtextextended('incoming-manual-reservation:' || v_doc, 0));
  if exists (
    select 1 from public.incoming_receipt_attempts
     where document_ref_normalized=v_doc and actor<>v_actor and acknowledged_at is null
       and state in ('reviewed','attempted','accepted','unknown','conflict')
  ) then
    raise exception 'Receipt document is already reserved by another operator';
  end if;
  if exists (
    select 1 from public.incoming_receipt_attempts
     where actor=v_actor and acknowledged_at is null and batch_id<>v_doc
       and state in ('reviewed','attempted','accepted','unknown','conflict')
  ) then
    raise exception 'A prior receipt requires reconciliation before another receipt can start';
  end if;
  ...
  select coalesce(max(source_line_no),0) + 1 into v_next
   ...
   where document_ref_normalized=v_doc and source_page_normalized='MANUAL';
  if v_next > 500 then raise exception 'Receipt manual line limit reached'; end if;
  v_expected := 'incoming:' || v_doc || '|MANUAL|' || v_next::text;

  insert into public.incoming_receipt_attempts (
    actor,document_ref,document_ref_normalized,source_page,source_page_normalized,
    source_line_no,part_number,part_number_canonical,qty,correlation_id,batch_id,state
  ) values (
    v_actor,btrim(p_document_ref),v_doc,'MANUAL','MANUAL',v_next,v_part,v_part,p_qty,v_expected,v_doc,'reviewed'
  ) returning * into v_attempt;

  insert into public.incoming_receipt_attempt_events
    (attempt_id,revision,event_type,from_state,to_state,actor,details)
  values (v_attempt.id,v_attempt.revision,'manual_reserved',null,'reviewed',v_actor,
    jsonb_build_object('correlationId',v_expected,'sourceLineNo',v_next));
  return public.incoming_receipt_attempt_payload(v_attempt);
end;
$$;
...
revoke all on function public.reserve_incoming_manual_receipt_review(text,text,text,integer) from public, anon, authenticated;
...
grant execute on function public.reserve_incoming_manual_receipt_review(text,text,text,integer) to service_role;
