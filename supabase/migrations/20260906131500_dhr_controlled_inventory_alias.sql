-- Controlled DHR inventory cross-reference.
--
-- Production parity inspection found one required controlled-form part whose DHR
-- identifier has no exact Stock Summary row: 5600 / 5.15 / J61239. Inventory
-- carries the packaged FRU as J61239P (FRU, TRUMOTION BELT), while the controlled
-- DHR form keeps J61239 (Belt, CM Rotor, geared Trumotion). Keep those identities
-- separate instead of weakening canonical matching or duplicating inventory rows.

ALTER TABLE public.dhr_expected_parts
  ADD COLUMN IF NOT EXISTS inventory_part_number text;

COMMENT ON COLUMN public.dhr_expected_parts.inventory_part_number IS
  'Optional authoritative Stock Summary part number consumed for this controlled DHR form line. NULL means use part_number.';
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'dhr_expected_parts_inventory_part_number_nonblank'
      AND conrelid = 'public.dhr_expected_parts'::regclass
  ) THEN
    ALTER TABLE public.dhr_expected_parts
      ADD CONSTRAINT dhr_expected_parts_inventory_part_number_nonblank
      CHECK (inventory_part_number IS NULL OR btrim(inventory_part_number) <> '');
  END IF;
END;
$$;

-- Guard the one evidenced production cross-reference. This changes no inventory
-- quantity and is intentionally conditional: refuse to seed the alias unless the
-- controlled DHR row exists exactly once, J61239 itself is absent from stock, and
-- J61239P exists exactly once. The update is reversible by setting the alias NULL.
DO $$
DECLARE
  v_expected_count integer;
  v_exact_stock_count integer;
  v_alias_stock_count integer;
BEGIN
  SELECT count(*) INTO v_expected_count
  FROM public.dhr_expected_parts
  WHERE analyzer_model = '5600'
    AND section_id = '5.15'
    AND upper(btrim(part_number)) = 'J61239';

  SELECT count(*) INTO v_exact_stock_count
  FROM public.stock
  WHERE upper(btrim(part_number)) = 'J61239';

  SELECT count(*) INTO v_alias_stock_count
  FROM public.stock
  WHERE upper(btrim(part_number)) = 'J61239P';

  IF v_expected_count = 1 AND v_exact_stock_count = 0 AND v_alias_stock_count = 1 THEN
    UPDATE public.dhr_expected_parts
    SET inventory_part_number = 'J61239P'
    WHERE analyzer_model = '5600'
      AND section_id = '5.15'
      AND upper(btrim(part_number)) = 'J61239'
      AND (inventory_part_number IS NULL OR upper(btrim(inventory_part_number)) = 'J61239P');
  END IF;
END;
$$;
...
DECLARE
  ...
  v_inventory_canonical_part text;
...
BEGIN
  ...
  -- The controlled-form identity remains the DHR/event identity. Inventory may
  -- use an explicit server-owned cross-reference, never a caller-supplied alias.
  v_canonical_part := upper(btrim(v_expected.part_number));
  v_inventory_canonical_part := upper(btrim(coalesce(nullif(v_expected.inventory_part_number, ''), v_expected.part_number)));
  ...
  END IF;
  IF v_inventory_canonical_part IS NULL OR v_inventory_canonical_part = '' THEN
    RAISE EXCEPTION 'Configured DHR inventory part is invalid';
  END IF;
  ...
  IF lower(v_category) <> 'tool' THEN
    ...
    FROM public.stock
    WHERE upper(btrim(part_number)) = v_inventory_canonical_part;
    IF v_stock_match_count = 0 THEN
      RAISE EXCEPTION 'Inventory part not found for controlled DHR part: %', p_part_number;
    ELSIF v_stock_match_count > 1 THEN
      RAISE EXCEPTION 'Ambiguous inventory part for controlled DHR part: %', p_part_number;
    END IF;
  ...
  IF v_result.id IS NULL THEN
    INSERT INTO public.dhr_scan_results(
      ...
      p_new_qty, v_category, v_status, v_stock_before, v_stock_after,
      CASE WHEN lower(v_category) = 'tool' THEN NULL ELSE v_stock.id END,
