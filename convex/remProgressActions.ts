...
function uuid(value:string) { if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) throw new ConvexError('Invalid record or engineer ID'); }
async function request(resource:string, body?:unknown) {
  const url=process.env.SUPABASE_URL; const key=process.env.SUPABASE_SERVICE_ROLE_KEY;
  if(!url||!key) throw new ConvexError('REM progress service is unavailable');
...
async function engineers() {
  ...
  if(!Array.isArray(rows)) throw new ConvexError('Engineer directory unavailable');
...
export const updateProgress=action({
  ...
  handler:async(ctx,args)=>{
    ...
    if(!Number.isSafeInteger(args.expectedRevision)||args.expectedRevision<0) throw new ConvexError('Invalid revision');
    if(args.notes.length>4000||! /^[A-Za-z0-9:._-]{1,180}$/.test(args.correlationId)) throw new ConvexError('Invalid notes or request identifier');
    try { validateProgress(args.kind,args.progress,args.stage); }
    catch (error) { throw new ConvexError(error instanceof Error ? error.message : 'Invalid REM progress'); }
