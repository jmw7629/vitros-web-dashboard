...
export const createAnalyzer = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const doc = { serialNumber: args.serialNumber.trim() };
    if (args.analyzerType !== undefined) (doc as Record<string, unknown>).analyzerType = args.analyzerType;
    if (args.stage !== undefined) (doc as Record<string, unknown>).stage = args.stage;
    if (args.progress !== undefined) (doc as Record<string, unknown>).progress = clampPct(args.progress);
    if (args.status !== undefined) (doc as Record<string, unknown>).status = args.status;
    if (args.notes !== undefined) (doc as Record<string, unknown>).notes = args.notes;
