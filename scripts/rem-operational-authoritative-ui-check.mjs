...
requireText(migration, /coalesce\(btrim\(v_audit\.old_value->>'stage'\), ''\)\s*<>\s*btrim\(p_expected_stage\)/, "idempotency expected-stage identity");
requireText(migration, /coalesce\(btrim\(v_audit\.old_value->>'notes'\), ''\)\s*<>\s*v_expected_notes/, "idempotency expected-notes identity");
requireText(migration, /coalesce\(v_audit\.new_value->>'stage', ''\)\s*<>\s*btrim\(p_stage\)/, "idempotency target-stage identity");
requireText(migration, /coalesce\(v_audit\.new_value->>'notes', ''\)\s*<>\s*coalesce\(v_notes, ''\)/, "idempotency target-notes identity");
