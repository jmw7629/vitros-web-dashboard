const TARGET_SHA = "b32fc388b11578b7b5b7bf49ef7ce02f5a4794a6";
...
const [ui, server, migration] = await Promise.all([
...
// This exact implementation intentionally requires a real Convex-authenticated
// server identity. It must never be represented as production-write-ready while
// the browser-only role picker remains the only user session mechanism.
requireAll(server, "REM auth boundary", ['requireCapability(ctx, "rem.write")']);

console.log(`VERIFY=PASS SHA=${TARGET_SHA} AUTH_DEPENDENCY=ISSUE_56`);
