-- Forward migration: extend public.inventory_operations with nullable requested_batch_id and requested_analyzer_serial.
-- Replace public.apply_inventory_transition so newly inserted operation rows persist p_batch_id/p_analyzer_serial.
-- A reused correlation compares canonical part, mode, requested_qty, requested_batch_id, and requested_analyzer_serial
-- before returning duplicate. Use IS DISTINCT FROM for nullable material fields.
-- Keep SECURITY DEFINER, search_path public, pg_temp, service_role-only EXECUTE.
-- Atomic stock -> audit -> pending SAP staging, no production SAP post, no destructive rewrite.
...
begin
  ...
  values (p_correlation_id,p_part_number,p_mode,p_qty,p_batch_id,p_analyzer_serial)
  ...
  if not found then
    ...
    -- Null-safe comparison for all material fields including new columns
    ...
    if v_existing.requested_batch_id is distinct from p_batch_id then
      raise exception 'Inventory idempotency conflict';
    end if;
    if v_existing.requested_analyzer_serial is distinct from p_analyzer_serial then
      raise exception 'Inventory idempotency conflict';
    end if;
