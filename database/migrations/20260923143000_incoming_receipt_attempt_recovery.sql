...
create table if not exists public.incoming_receipt_attempt_events (
  id uuid primary key default gen_random_uuid(),
  attempt_id uuid not null references public.incoming_receipt_attempts(attempt_id),
  event_type text not null check (event_type in ('reviewed','abandoned','re_reviewed','material_conflict')),
...
begin
  ...
  if found then
    ...
    insert into public.incoming_receipt_attempts(
      ...
      gen_random_uuid(), v_correlation, p_part_number, p_source_page, p_source_line_no,
