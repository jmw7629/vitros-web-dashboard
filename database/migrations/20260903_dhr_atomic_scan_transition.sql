...
-- This migration is additive and does not post anything to SAP.
...
begin
  ...
  if p_expected_qty is null or p_expected_qty < 0 then raise exception 'expected quantity must be zero or greater'; end if;
  ...
  if v_result_match_count > 1 then raise exception 'Ambiguous canonical DHR result for part %', p_part_number; end if;
  ...
  elsif p_new_qty = p_expected_qty then v_status := 'matched';
  elsif p_new_qty < p_expected_qty then v_status := 'short';
  else v_status := 'over';
  end if;
  ...
  if lower(p_category) <> 'tool' then
    ...
    if v_stock_match_count = 0 then raise exception 'Part not found: %', p_part_number;
    elsif v_stock_match_count > 1 then raise exception 'Ambiguous canonical stock part: %', p_part_number;
    end if;
    ...
    if v_delta > 0 then
      ...
      v_inventory := public.apply_inventory_transition(
        ...
        p_analyzer_serial, p_session_id::text
      );
  ...
  if v_result.id is null then
    ...
    set description = p_description,
        ...
        scanned_at = now(), scanned_by = p_actor, updated_at = now(), revision = v_revision_after
