...
## 2026-09-23 — issue449 J05600 concurrency repair

Independent exact-head review of PR435 reproduced a real stale-config race: the prior `SHARE` lock allowed an `apply_dhr_scan_transition` writer that had read section5.10 to wait on stock while the migration moved expected configuration to5.12, then commit immutable5.10 history. The repair adopts the existing Cycle Count/DHR lock-order precedent by taking `EXCLUSIVE` on `dhr_scan_sessions` first in both forward and guarded reverse, retaining the five-second lock timeout and all narrow row/history guards.

A new actual-path PostgreSQL17 regression holds the J05600 stock row, confirms the real DHR writer is waiting on `Lock:transactionid`, then executes the literal migration/reverse concurrently. Both directions fail closed on lock timeout without changing expected configuration; after releasing the blocker the real writer completes at its original section. The original39/39 serial suite, DHR deletion tests and Cycle Count SQL regression also pass. No Production data, SAP posting, service, credential or controlled document was changed; exact-head GitHub CI/READY preview and fresh independent review remain required before merge or any Production application.
