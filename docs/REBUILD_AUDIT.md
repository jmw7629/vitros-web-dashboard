...
**Architecture**: The installer always provisions a dedicated, clean verifier control clone at `$HOME/.local/share/joeos-opencode-bridge/vitros-verifier-control`, regardless of any builder `BRIDGE_ROOT` value inherited from the shared environment. The control clone must have the expected GitHub origin and a clean working tree, is pinned to the exact installer commit, and is never replaced destructively. The verifier runner (`verifier_runner.py:main()`) gives CLI `--root` precedence over `BRIDGE_ROOT`, so the systemd service executes `python3 <dedicated-path>/bridge/verifier_gate_runner.py --root <dedicated-path>`. The live builder checkout is never mutated, reset, stashed, cleaned, or used as verifier control.

**Recovery rule**: Existing builder environment values cannot redirect the verifier back to the live checkout. A missing dedicated control clone is created; an existing dirty, non-git, or wrong-origin control path fails closed instead of being deleted. The systemd drop-in `20-isolated-control.conf` is no longer needed as a workaround, since the installer embeds this topology.
