...
def process(root:Path,repo:str,i:dict[str,Any],state:dict[str,Any],state_path:Path,wtroot:Path):
    ...
    p=run(cmd,cwd=wt,check=False,env=env); log.write_text(f"Model source: {model_source}\nModel: {model or '(OpenCode default)'}\n$ {shlex.join(cmd[:-1])} <PROMPT>\n\nexit={p.returncode}\n\nSTDOUT\n{p.stdout or ''}\n\nSTDERR\n{p.stderr or ''}\n"); os.chmod(log,0o600)
    if p.returncode!=0:
